...
def simulator(sample_path):
    ...
    '''指令部分和数据部分预处理'''
    # 将指令装为{地址:指令}字典; 将数据装为{地址:值}字典
    mergeDict = lambda x, y: dict(x, **y)                           # 将两个字典合并
    inspart_dict = reduce(mergeDict, map(insToDict, instructions))  # 将所有字典合并
    datapart_dict = reduce(mergeDict, map(dataToDict, l[axis:]))
...
def simulatorExe(inspart_dict, datapart_dict):
    ...
    insLen = len(inspart_dict) / 2
    dataLen = len(datapart_dict)
    firstDataAddress = instr_address + 4 * insLen
    ...
    while is_execute:
        ...
        if dataLen > 8:
            ...
            for x in data_sorted_list[8:16]: sim_list += '\t' + str(datapart_dict[x])
        sim_list += '\n\n'
